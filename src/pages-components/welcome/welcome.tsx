  ...
  useEffect(() => {
    if(initData?.user?.username != null){
      ...
      .then(data => {
        ...
        if (data.userInfo.vote_data) {
          setVotes(data.userInfo.vote_data.split(',')); // assuming votes are stored in vote_data as comma-separated values
        } else {
          setVotes([]); // Oder ein anderer Standardwert
        }
