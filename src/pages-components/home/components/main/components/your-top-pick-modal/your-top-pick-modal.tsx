...
import {selectStore} from '../../../../../../stores/store'

const workSans = Work_Sans({ subsets: ['latin'] })

export const YourTopPickModal = ({ open, close }: Props) => {
  const name = selectStore((state) => state.name);
  const src = selectStore((state) => state.src);
  return (
    <>
      <div
        onClick={close}
      ...
      >
        <div>
          <div className='flex justify-center w-full'>
            <div className='w-[132px] relative h-[132px] bg-gray-300 rounded-[6px]'>
              <Image
                alt= {name}
                src= {src} 
                width={132}
